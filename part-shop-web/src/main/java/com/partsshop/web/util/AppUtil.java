 ...
 * 
 */
package com.partsshop.web.util;
...
public class AppUtil {
	...
	public static String buildPaginatedQueryRest(PaginationCriteria paginationCriteria) {
		if(!AppUtil.isObjectEmpty(paginationCriteria)) {
			int pnum = paginationCriteria.getPageNumber() == 0? 0 : paginationCriteria.getPageNumber() / paginationCriteria.getPageSize() ; 
			return String.format("?start=%s&count=%s", pnum, 
					paginationCriteria.getPageSize().toString()) ;
		}else {
			return null ; 
		}
