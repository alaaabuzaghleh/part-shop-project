...
public class DataTableRequest<T> {
	...
	/** The order. */
	private List<DataTableOrderSpecs> order;
	 ...
	 * @return the order
	 */
	public List<DataTableOrderSpecs> getOrder() {
		return order;
	}
	 ...
	 */
	public void setOrder(List<DataTableOrderSpecs> order) {
		this.order = order;
	}
...
//	}
	...
	public PaginationCriteria getPaginationRequest() {
		...
		if(!AppUtil.isObjectEmpty(this.getOrder())) {
			sortBy = new SortBy();
			for(DataTableOrderSpecs order : this.getOrder())
				sortBy.addSort(String.valueOf(order.getColumn()), SortOrder.fromValue(order.getDir()));
		}
		...
		for(DataTableColumnSpecs colSpec : this.getColumns()) {
			if(colSpec.isSearchable()) {
				if(!AppUtil.isObjectEmpty(this.getSearch()) || !AppUtil.isObjectEmpty(colSpec.getSearch())) {
					filterBy.addFilter(colSpec.getData(), (this.search.getValue()!=null) ? this.getSearch().getValue() : colSpec.getSearch().getValue());
				}
			}
		}
